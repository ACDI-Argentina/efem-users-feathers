...
const projects = (app, liquidPledging) => {
  ...
  function findToken(address) {
    ...
    const token = tokenWhitelist.find(t => t.address.toLowerCase() === address.toLowerCase());

    if (!token) throw new Error(`Un-whitelisted token: ${address}`);
  ...
  async function getLppCappedMilestoneBase() {
    return getKernel().then(kernel =>
      kernel.getApp(keccak256('base'), keccak256('lpp-capped-milestone')),
    );
  }
